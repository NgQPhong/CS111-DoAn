...
class Memory(Textbase):
    def __init__(self, app):
        self.frame = Frame(app.app)

        self.name = Text(self.frame, wrap=WORD, font=app.font, fg="#237893", bg=app.color["bg"], width=0, height=1,
                         highlightthickness=0, bd=0)
        self.name.insert(1.0, "Memory")
        self.name.config(state=DISABLED)
        self.name.pack(side=TOP, fill=BOTH)

        self.number = Text(self.frame, wrap=WORD, font=app.font, fg="#237893", bg=app.color["bg"], width=0, height=1,
                         highlightthickness=0, bd=0)
        ...
        self.index = Text(self.frame, wrap=CHAR, font=app.font, fg="#237893", bg=app.color["bg"], width=4, height=0,
                          highlightthickness=0, bd=0)
        self.index.insert(1.0, "  1 ")
        self.index.config(selectbackground=self.index.cget("bg"), state=DISABLED)

        self.text = Text(self.frame, wrap=NONE, font=app.font, fg=app.color["fg"], bg=app.color["bg"], undo=True,
                         width=100, height=0, highlightthickness=0, bd=0)
    ...
    def render(self):
        self.frame.pack(side=RIGHT, fill=BOTH, pady=(2, 0))